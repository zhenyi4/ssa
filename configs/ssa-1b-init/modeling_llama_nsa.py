...
from .configuration_llama_nsa import LlamaNSAConfig
...
class LlamaRotaryEmbedding(nn.Module):
    def __init__(self, config: LlamaNSAConfig, device=None):
        super().__init__()
...
class LlamaNSAAttention(nn.Module):
    ...
    def __init__(self, config: LlamaNSAConfig, layer_idx: int):
        super().__init__()
        self.config = config
        ...
        # new for NSA
        self.g_proj = nn.Linear(config.hidden_size, config.num_attention_heads * 3, bias=False)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # new for NSA
        gate = self.g_proj(hidden_states).view(*input_shape, -1, 3)
        g_cmp, g_slc, g_swa = gate.sigmoid().unbind(-1)

        cos, sin = position_embeddings
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin) 
        # query_states, key_states = liger_rotary_pos_emb(query_states, key_states, cos.transpose(1, 2), sin.transpose(1, 2))
...
class LlamaNSADecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: LlamaNSAConfig, layer_idx: int):
        ...
        self.self_attn = LlamaNSAAttention(config=config, layer_idx=layer_idx)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # Self Attention
        hidden_states, _, sa_loss = self.self_attn(
        ...
        hidden_states = residual + hidden_states
        return hidden_states, sa_loss


@auto_docstring
class LlamaNSAPreTrainedModel(PreTrainedModel):
    config: LlamaNSAConfig
...
import random
@dataclass
class BaseModelOutputWithPastAndSALoss(BaseModelOutputWithPast):
    sa_loss:  Optional[torch.Tensor] = None

class LlamaNSAModel(LlamaNSAPreTrainedModel):
    def __init__(self, config: LlamaNSAConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaNSADecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        for layer, decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            hidden_states, sa_loss = decoder_layer(
                hidden_states,
        ...
        total_sa_loss /= self.config.num_hidden_layers
        hidden_states = self.norm(hidden_states)
        return BaseModelOutputWithPastAndSALoss(
            last_hidden_state=hidden_states,
            past_key_values=past_key_values,
            sa_loss=total_sa_loss
        )


@auto_docstring
class LlamaNSAForCausalLM(LlamaNSAPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = LlamaNSAModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if self.training:
            ...
            self._last_cross_entropy_loss = loss.detach()
            self._last_alignment_loss = outputs.sa_loss.detach()
...
class LlamaNSAForSequenceClassification(GenericForSequenceClassification, LlamaNSAPreTrainedModel): ...


class LlamaNSAForQuestionAnswering(GenericForQuestionAnswering, LlamaNSAPreTrainedModel):
    base_model_prefix = "transformer"  # For BC, where `transformer` was used instead of `model`


class LlamaNSAForTokenClassification(GenericForTokenClassification, LlamaNSAPreTrainedModel): ...


__all__ = [
    "LlamaNSAForCausalLM",
    "LlamaNSAModel",
    "LlamaNSAPreTrainedModel",
